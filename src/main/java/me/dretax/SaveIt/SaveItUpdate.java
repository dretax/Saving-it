...
public class SaveItUpdate {
	 ...
	 */
	public SaveItUpdate(Plugin plugin, int id, File file, UpdateType type, boolean announce) {
		this.plugin = plugin;
		this.type = type;
		...
		try {
			this.url = new URL(SaveItUpdate.HOST + SaveItUpdate.QUERY + id);
	...
	private void saveFile(File folder, String file, String u) {
		...
		try {
			...
			final byte[] data = new byte[SaveItUpdate.BYTE_SIZE];
			int count;
			if (this.announce) {
				this.plugin.getLogger().info("About to download a new update: " + this.versionName);
			}
			long downloaded = 0;
			while ((count = in.read(data, 0, SaveItUpdate.BYTE_SIZE)) != -1) {
				downloaded += count;
			...
			this.result = SaveItUpdate.UpdateResult.FAIL_DOWNLOAD;
		} finally {
			try {
				if (in != null) {
					in.close();
		...
		try {
			...
			while (e.hasMoreElements()) {
				...
				if (entry.isDirectory()) {
					...
					int b;
					final byte buffer[] = new byte[SaveItUpdate.BYTE_SIZE];
					final FileOutputStream fos = new FileOutputStream(destinationFilePath);
					final BufferedOutputStream bos = new BufferedOutputStream(fos, SaveItUpdate.BYTE_SIZE);
					while ((b = bis.read(buffer, 0, SaveItUpdate.BYTE_SIZE)) != -1) {
		...
		if (this.type != UpdateType.NO_VERSION_CHECK) {
			...
			if (title.split(" v").length == 2) {
				...
				if (this.hasTag(version) || version.equalsIgnoreCase(remoteVersion)) {
					...
					this.result = SaveItUpdate.UpdateResult.NO_UPDATE;
					return false;
				}
			} else {
				...
				this.result = SaveItUpdate.UpdateResult.FAIL_NOVERSION;
				return false;
			}
		}
		return true;
	...
	private boolean hasTag(String version) {
		for (final String string : SaveItUpdate.NO_UPDATE_TAG) {
	...
	private boolean read() {
		try {
			...
			this.versionName = (String) ((JSONObject) array.get(array.size() - 1)).get(SaveItUpdate.TITLE_VALUE);
			this.versionLink = (String) ((JSONObject) array.get(array.size() - 1)).get(SaveItUpdate.LINK_VALUE);
			this.versionType = (String) ((JSONObject) array.get(array.size() - 1)).get(SaveItUpdate.TYPE_VALUE);
			this.versionGameVersion = (String) ((JSONObject) array.get(array.size() - 1)).get(SaveItUpdate.VERSION_VALUE);
	...
	private class UpdateRunnable implements Runnable {
		...
		@Override
		public void run() {
			if (SaveItUpdate.this.url != null) {
				// Obtain the results of the project's file feed
				if (SaveItUpdate.this.read()) {
					if (SaveItUpdate.this.versionCheck(SaveItUpdate.this.versionName)) {
						if ((SaveItUpdate.this.versionLink != null) && (SaveItUpdate.this.type != UpdateType.NO_DOWNLOAD)) {
							String name = SaveItUpdate.this.file.getName();
							// If it's a zip file, it shouldn't be downloaded as the plugin's name
							if (SaveItUpdate.this.versionLink.endsWith(".zip")) {
								final String[] split = SaveItUpdate.this.versionLink.split("/");
								name = split[split.length - 1];
							}
							SaveItUpdate.this.saveFile(new File(SaveItUpdate.this.plugin.getDataFolder().getParent(), SaveItUpdate.this.updateFolder), name, SaveItUpdate.this.versionLink);
						} else {
							SaveItUpdate.this.result = UpdateResult.UPDATE_AVAILABLE;
						}
					}
				}
			}
